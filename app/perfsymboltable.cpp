                                      ...
                                      qint32 inlineCallLocationId, bool isKernel)
{
    ...
    qint32 symId = m_unwind->resolveString(demangle(cudie->dieName(top)));
    qint32 mangleId = m_unwind->resolveString(cudie->dieName(top));
...
static PerfAddressCache::SymbolCache cacheSymbols(Dwfl_Module *module, quint64 elfStart, bool isArmArch)
{
    ...
    for (int i = 0; i < numSymbols; ++i) {
        ...
        if (symbol) {
            ...
            cache.append({symAddr - elfStart, start, size, symbol, relocated, adjust});
        }
    }
    return cache;
}
    ...
    if (mod) {
        ...
        if (!m_unwind->hasSymbol(addressLocation.parentLocationId)) {
            ...
            m_unwind->resolveSymbol(addressLocation.parentLocationId,
                                    PerfUnwind::Symbol(symId, mangleId, start, size, binaryId, binaryPathId, isKernel));
        }
    } else {
        if (isKernel) {
