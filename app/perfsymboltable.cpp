...
static quint64 alignedAddress(quint64 addr, bool isArmArch, Direction direction)
{
    return (direction == Fwd) ? symbolAddress(addr, isArmArch) : alignedAddress(addr, isArmArch);
}
...
static PerfAddressCache::SymbolCache cacheSymbols(Dwfl_Module *module, quint64 elfStart, bool isArmArch)
{
    ...
    for (int i = 0; i < numSymbols; ++i) {
        ...
        if (symbol) {
            quint64 start = alignedAddress(sym.st_value, isArmArch, Back);
                                 ...
                                 bool *isInterworking)
{
    ...
    PerfUnwind::Location addressLocation(alignedAddress(ip, isArmArch, Fwd), 0, -1, m_pid);
    ...
    if (mod) {
        ...
        if (cachedAddrInfo.isValid()) {
            ...
            relAddr = alignedAddress(start + off, isArmArch, Back);
